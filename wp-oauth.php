...
Class WPOA {
	...
	private $settings = array(
		...
		'wpoa_new_user_role' => 'subscriber',							// role
	...
	*/
	...
	function init() {
		...
		if (get_option('wpoa_show_login_messages') !== false) {
			...
			add_filter('login_footer', array($this, 'wpoa_push_login_messages'));
                ...
                add_action('wp_loaded', array($this, 'wpoa_update_user_data'));
	}
	...
	function wpoa_login_user($oauth_identity) {
		...
		$_SESSION["WPOA"]["USER_ID"] = $oauth_identity["id"];
                $_SESSION["WPOA"]["WPOA_UPDATE_DATA"] = $oauth_identity;
	...
	function wpoa_settings_page_content() {
        ...
        function wpoa_update_user_data(){
            if (!empty($_SESSION['WPOA']['WPOA_UPDATE_DATA'])){
                
                switch($_SESSION['WPOA']['PROVIDER']){
                    case 'fiware':
                        $data = $_SESSION['WPOA']['WPOA_UPDATE_DATA'];
                        ...
                        list($firstName, $lastName) = explode(" ",$data['displayName']);
                        update_user_meta($user->ID,'nickname', $data['nickName']);
                        update_user_meta($user->ID,'first_name', $firstName);
                        update_user_meta($user->ID,'last_name', $lastName);
                        
                        if(wp_update_user($user_data)){
                            unset($_SESSION['WPOA']['WPOA_UPDATE_DATA']);
                            unset($_SESSION['WPOA']['UPDATE_DATA']);                            
                        }
                        break;
