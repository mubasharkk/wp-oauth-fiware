...
define('URL_AUTH', "https://account.lab.fiware.org/oauth2/authorize?");
define('URL_TOKEN', "https://account.lab.fiware.org/oauth2/token?");
define('URL_USER', "https://account.lab.fiware.org/user?");
...
//	parse_str($result, $result_obj); // PROVIDER SPECIFIC: Facebook encodes the access token result as a querystring by default
        $result_obj = (array) json_decode($result);
...
function get_oauth_identity($wpoa) {
	...
	$oauth_identity['nickName'] = $result_obj['nickName']; //PROVIDER SPECIFIC: this is how Facebook returns the email address
	$oauth_identity['displayName'] = $result_obj['displayName']; //PROVIDER SPECIFIC: this is how Facebook returns the email address
